...
function eventListenerTstamp(){
    ...
    $timestamp.addEventListener("click", (e) => {
        if(cnt===0){
            ...
            let today = new Date();
            let nowTime = today.toLocaleString();
            console.log(nowTime);
            $newTstamp.textContent = nowTime;
            $newTstamp.classList.add("now-time");
            const $timeTarget = document.querySelector(".page");
            $timeTarget.parentElement.appendChild($newTstamp);
            cnt=1;
        }
        if(cnt===1){
            const $modifTime = document.querySelector(".now-time");
            
            console.log("checkOne")
            let today2 = new Date();
            let nowTime2 = today2.toLocaleString();
            console.log("nowTime2" + nowTime2);
            $modifTime.textContent = nowTime2;
        }
