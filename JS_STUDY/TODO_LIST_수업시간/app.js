...
function renderNewTodo(newTodo) {
    ...
    const $newLi = document.createElement('li');  //깡통 li 만듬
    $newLi.classList.add('todo-list-item'); //li 디장니 한게 있으니 클래스 붙이고
    $newLi.dataset.id = newTodo.id; //data-id 붙이고
...
//할 일 추가 기능 처리
// 본래는 DB로도 데이터를 보내는 과정이 여기에 포함된다.
